    ...
    <form method="post" action="{{ route('patterns.update', $pattern) }}">
        ...
        <div class="row" style="margin-top:10px;">
            <div class="field">
                ...
                <input id="popularity_rank" name="popularity_rank" type="number" min="1" value="{{ old('popularity_rank', $pattern->popularity_rank) }}" required>
                @error('popularity_rank')<div class="error">{{ $message }}</div>@enderror
            </div>
            <div class="field">
                <label for="min_total_length">Min total length</label>
                <input id="min_total_length" name="min_total_length" type="number" min="0" value="{{ old('min_total_length', $pattern->min_total_length) }}" required>
                @error('min_total_length')<div class="error">{{ $message }}</div>@enderror
            </div>
            <div class="field">
                <label for="forename_count">Forename count</label>
                <input id="forename_count" name="forename_count" type="number" min="0" value="{{ old('forename_count', $pattern->forename_count) }}" required>
                @error('forename_count')<div class="error">{{ $message }}</div>@enderror
            </div>
            <div class="field">
                <label for="surname_count">Surname count</label>
                <input id="surname_count" name="surname_count" type="number" min="0" value="{{ old('surname_count', $pattern->surname_count) }}" required>
                @error('surname_count')<div class="error">{{ $message }}</div>@enderror
            </div>
        </div>
        <div class="flags" style="margin-top:10px;">
            <label><input type="checkbox" name="has_title" value="1" {{ old('has_title', $pattern->has_title) ? 'checked' : '' }}> title</label>
            <label><input type="checkbox" name="has_initials" value="1" {{ old('has_initials', $pattern->has_initials) ? 'checked' : '' }}> initials</label>
            <label><input type="checkbox" name="has_prefix" value="1" {{ old('has_prefix', $pattern->has_prefix) ? 'checked' : '' }}> prefix</label>
            <label><input type="checkbox" name="has_suffix" value="1" {{ old('has_suffix', $pattern->has_suffix) ? 'checked' : '' }}> suffix</label>
            <label><input type="checkbox" name="has_honorific" value="1" {{ old('has_honorific', $pattern->has_honorific) ? 'checked' : '' }}> honorific</label>
        </div>
