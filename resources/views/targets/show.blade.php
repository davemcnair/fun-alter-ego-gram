...
<script>
    ...
    (function () {
        try {
            console.log('[UI] Target page script boot');
        } catch (e) {
        }
        ...
        try {
            window.onerror = function (msg, src, line, col, err) {
                try {
                    console.error('[UI] window.onerror', msg, src, line, col, err);
                } catch (e) {
                }
            };
            window.onunhandledrejection = function (ev) {
                try {
                    console.error('[UI] unhandledrejection', ev && ev.reason ? ev.reason : ev);
                } catch (e) {
                }
            };
        } catch (e) {
        ...
        (function () {
            ...
            btn.addEventListener('click', async function () {
                const rows = Array.from(document.querySelectorAll('#deferredPatternsTable tbody tr'));
                ...
                const runNext = async () => {
                    ...
                    const pid = ids[i++];
                    try {
                        await window.searchPattern(pid);
                    } catch (e) {
                    }
        ...
        if (showElapsedToggle) {
            try {
                ...
                showElapsedToggle.addEventListener('change', function () {
                    ...
                    try {
                        localStorage.setItem('showElapsedToggle', showElapsed ? '1' : '0');
                    } catch (e) {
                    }
                    window.rerender && window.rerender();
                });
        ...
        async function call(url, method = 'POST') {
            ...
            if (res.status === 419 || res.status === 401) {
                return {
                    ok: false,
                    error: 'Your session expired or CSRF token is invalid. Please reload the page and try again.'
                };
            }
            if (res.redirected) {
        ...
        function appendGroupToDom(g) {
            ...
            const list1 = onlyFun ? all.filter(function (ph) {
                return hasAnyFunToken(ph, g.pattern);
            }) : all;
            const list1b = onlyStarred ? list1.filter(function (ph) {
                return starredSet.has(ph);
            }) : list1;
            const list = (wordFilter.word ? list1b.filter(function (ph) {
                return phraseContainsWord(ph, g.pattern, wordFilter.word, wordFilter.token);
            }) : list1b);
        ...
        window.toggleGroupExpand = function (pattern, expand) {
            try {
                groupExpanded[String(pattern)] = !!expand;
            } catch (e) { /* ignore */
            }
            if (__lastProgress) {
                render(__lastProgress);
            }
        }
        ...
        function computeUsedWordSetsFromGroups(groups) {
            ...
            try {
                ...
                arr.forEach(function (g) {
                    ...
                    const filtered1 = onlyFun ? allPhrases.filter(function (ph) {
                        return hasAnyFunToken(ph, g.pattern);
                    }) : allPhrases;
                    const filtered1b = onlyStarred ? filtered1.filter(function (ph) {
                        return starredSet.has(ph);
                    }) : filtered1;
                    const filtered = (wordFilter.word ? filtered1b.filter(function (ph) {
                        return phraseContainsWord(ph, g.pattern, wordFilter.word, wordFilter.token);
                    }) : filtered1b);
                ...
                nodes.forEach(function (n) {
                    ...
                    if (isUsed) {
                        ...
                        if (token === 'forename' || token === 'surname') {
                            ...
                            n.onclick = function () {
                                window.setWordFilter(n.getAttribute('data-word') || '', token);
                            };
                        }
                    }
        ...
        function render(p) {
            try {
                renderMatchedWords(p && p.matchedWords ? p.matchedWords : {});
            } catch (e) { /* ignore */
            }
            __lastProgress = p;
            ...
            const groupsArr = Array.isArray(p && p.patternsLive) ? (p.patternsLive.map(function (pl) {
                ...
                const phrases = Array.isArray(pl && pl.alterEgos) ? pl.alterEgos.map(function (ae) {
                    return (ae && ae.phrase) ? ae.phrase : (ae && ae['phrase'] ? ae['phrase'] : '');
                }).filter(Boolean) : [];
                const elapsedMs = pl && (pl.elapsed_ms ?? pl['elapsed_ms']);
                return {pattern: tmpl || '', phrases: phrases, elapsedMs: elapsedMs};
            }).filter(function (g) {
                return g.phrases.length > 0;
            })) : [];
            ...
            try {
                ...
                if (funAeFound) funAeFound.textContent = '0';
                if (patternsWithFunAE) patternsWithFunAE.textContent = '0';
            }
        ...
        (function () {
            ...
            if (TERMINAL.includes(lastStatus)) {
                try {
                    console.log('[progress] initial status is terminal; ensuring final render…', lastStatus);
                } catch (e) {
                }
                try {
                ...
                try {
                    ...
                    try {
                        console.log('[progress]', {
                            status: st,
                            completed: j.patterns.completed,
                            total: j.patterns.total,
                            alterEgos: j.alterEgosCount
                        });
                    } catch (e) {
                    }
                    if (TERMINAL.includes(st)) {
                        ...
                        if (!TERMINAL.includes(lastStatus)) {
                            try {
                                console.log('[progress] terminal status reached; reloading…', st);
                            } catch (e) {
                            }
                            ...
                            try {
                                console.log('[progress] terminal status confirmed; no reload');
                            } catch (e) {
                            }
    ...
    (function () {
        ...
        function esc(s) {
            return safe(s).replace(/[&<>]/g, function (c) {
                return c === '&' ? '&amp;' : (c === '<' ? '&lt;' : '&gt;');
            });
        }
        ...
        function setError(msg, source) {
            ...
            try {
                if (src) {
                    errorsEl.setAttribute('data-error-source', src);
                } else {
                    errorsEl.removeAttribute('data-error-source');
                }
            } catch (e) { /* ignore */
            }
            try {
                if (full) {
                    ...
                    errorsEl.setAttribute('hidden', 'hidden');
                }
            } catch (e) { /* ignore */
            }
        }
        ...
        window.renderMatchedWords = function (mw) {
            ...
            try {
                ...
                if (tokens.length === 0) {
                    wrap.innerHTML = '<div class="muted">No word matches found.</div>';
                    return;
                }
                let html = '';
        ...
        async function __handleAddWordSubmit(ev) {
            ...
            if (!payload.word) {
                setError('Please enter a word.', 'Client validation');
                setBusy(false);
                if (wordInput) wordInput.focus();
                return false;
            }
            if (!payload.token_type) {
                setError('Please select a token type.', 'Client validation');
                setBusy(false);
                if (tokenSel) tokenSel.focus();
                return false;
            }
            try {
                try {
                    console.log('[UI] POST add-word', ADD_WORD_URL(), payload);
                } catch (e) {
                }
                ...
                if (res && res.ok) {
                    ...
                    try {
                        ...
                        if (m.includes('csrf') || m.includes('session')) {
                            reason = 'Auth/CSRF';
                        }
                    } catch (e) { /* ignore */
                    }
                    try {
                        console.warn('[UI] Add word failed', {reason, res});
                    } catch (e) {
                    }
                    setError(msg, reason);
                }
                ...
                try {
                    console.error('[UI] Add word network error', e);
                } catch (_e) {
                }
    ...
    (function () {
        try {
            ...
            function normalizeSeparators(block) {
                ...
                Array.from(block.querySelectorAll('.ph-sep')).forEach(function (n) {
                    n.remove();
                });
            ...
            rows.forEach(function (row) {
                ...
                container.addEventListener('dragend', function (e) {
                    ...
                    Array.from(container.children).forEach(function (ch) {
                        ch.classList.remove('drop-target');
                    });
                });
