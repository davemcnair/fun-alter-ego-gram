...
class HelpsMatchWordsTest extends TestCase
{
    ...
    public function test_union_can_fill_slot_aware_upper_bound(): void
    {
        ...
        $this->assertTrue($this->helper->canFillFromAvailableLetterPools($tokenPrecomputed, $need));
        $needTooMuch = ['a'=>3,'c'=>1,'i'=>2,'n'=>1,'v'=>1];
        $this->assertFalse($this->helper->canFillFromAvailableLetterPools($tokenPrecomputed, $needTooMuch));
    }
}
