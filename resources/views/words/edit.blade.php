...
<div class="container">
    <div class="card">
        <h2 style="margin-top:0;">Edit Word #{{ $word->id }}</h2>
        <form method="post" action="{{ route('words.update', $word) }}">
            ...
            <div class="field">
                ...
                <input type="text" id="token_type" name="token_type" value="{{ old('token_type', $word->token_type) }}">
                @error('token_type')<div class="error">{{ $message }}</div>@enderror
            </div>
            <div class="field">
                <label for="list_type">List type</label>
                <input type="text" id="list_type" name="list_type" value="{{ old('list_type', $word->list_type) }}">
